# -*- coding: utf-8 -*-
"""
Created on Thu Feb 19 12:12:59 2026
...
# Define file names
root_path = r"C:\Users\KostasPikounis\OneDrive_Inlecom_Personal\OneDrive - INLECOM\Amfitrite\task2\OWD\Occurrences of HAB species in the Mediterranean"
occurrence_file = 'occurrence.txt'
mof_file = 'measurementorfact.txt'
output_data = 'homogenized_dataset_3.csv'
output_summary = 'dataset_3_summary.csv'

def process_and_homogenize():
    ...
    # According to meta.xml, occurrence.txt has 'id' (index 0) and measurementorfact.txt has 'coreid' (index 0)
    occ_id_col = df_occ.columns[0]
    mof_id_col = df_mof.columns[0]
    
    print(f"Merging Occurrence ({occ_id_col}) and MeasurementOrFact ({mof_id_col})...")
    df_merged = pd.merge(df_occ, df_mof, left_on=occ_id_col, right_on=mof_id_col, how='left')
    
    # 2. HOMOGENIZE COLUMN NAMES
    # Rename columns to match the 1st dataset summary
    rename_mapping = {
        occ_id_col: 'id_x',
    ...
    if 'eMoF_measurementUnit' in df_merged.columns and 'eMoF_measurementValue' in df_merged.columns:
        ...
        # Identify rows where the unit implies "/ml"
        ...
        # Populate those fields to match the core organismQuantity fields from dataset 1
...
if __name__ == "__main__":
    try:
        process_and_homogenize()
    except FileNotFoundError as e:
        print(f"Error: {e}. Please check if the files are in the directory.")
