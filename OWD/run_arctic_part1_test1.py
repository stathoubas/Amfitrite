# -*- coding: utf-8 -*-
"""
Created on Sat Mar  7 00:49:02 2026
...
def run_clustered_prescreener(csv_path, output_dir, x_days, y_days, purity_days, n_cases):
    ...
    # 1. Standardize Columns & Parse Data Safely for the New Dataset
    ...
    if not qty_col:
        logging.error("Could not find a quantity column (individualCount or organismQuantity). Exiting.")
        return

    # --- THE FIX: Parse Dates and Drop NaT immediately ---
    initial_len = len(df)
    ...
    if len(df) < initial_len:
        logging.info(f"Dropped {initial_len - len(df)} rows due to missing or invalid dates (NaT).")
    ...
    for seed_idx, seed_row in df_sorted.iterrows():
        ...
        # Bulletproof check just in case a NaT slipped through
        if pd.isna(seed_row['Event_Date_Parsed']): continue
        ...
        for cand_idx, cand_row in cluster_habs.iterrows():
            ...
            if pd.isna(cand_date): continue # Safety net
            
            logging.info(f"  [HAB Search] Trying ID {cand_row[id_col]} | Qty: {cand_row['qty_numeric']} | Date: {cand_date.date()}")
        ...
        for cand_idx, cand_row in cluster_cleans.iterrows():
            ...
            logging.info(f"  [Clean Search] Trying ID {cand_row[id_col]} | Qty: {cand_row['qty_numeric']} | Date: {cand_date.date()}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Grid-Based Clustered HAB API Pre-Screener (New Dataset)")
