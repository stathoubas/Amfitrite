...
class HABLightningModel(L.LightningModule):
    ...
    def _build_model(self, mode, weights_path):
        print(f"--- Building ConvNeXt V2 Base (Mode: {mode}) ---")
        
        # 1. Create the Skeleton with 10 Input Channels and 2 Output Classes
        # 'in_chans=10' automatically modifies the first layer for us!
        ...
        if mode == 'bigearthnet':
            ...
            # Unwrap nested dictionaries
            if 'state_dict' in state_dict: state_dict = state_dict['state_dict']
            elif 'model_state_dict' in state_dict: state_dict = state_dict['model_state_dict']
            
            new_state_dict = {}
            for k, v in state_dict.items():
                # Clean commonly found prefixes in BIFOLD/HuggingFace weights
                name = k.replace('module.', '').replace('backbone.', '').replace('model.vision_encoder.', '')
                
                # IMPORTANT: The BIFOLD weights have 19 classes. We have 2.
                # The head weights (head.fc.weight/bias) will have shape mismatches.
                # We SKIP them so our initialized 2-class head stays random.
                if "head." in name or "fc." in name:
                    continue
                    
                new_state_dict[name] = v
            
            # Load weights. strict=False is REQUIRED because we skipped the head.
            missing, unexpected = model.load_state_dict(new_state_dict, strict=False)
            
            print("Weights Loaded.") 
            # Verification: Check if stem (first layer) loaded
            # In ConvNeXt, the first layer is usually named 'stem.0.weight'
            if 'stem.0.weight' in missing:
                print("CRITICAL WARNING: stem.0.weight (First Layer) was NOT loaded!")
            else:
                print("SUCCESS: ConvNeXt Stem loaded successfully!")
...
if __name__ == "__main__":
    ...
    output_path = "/home/kostas/AMFITRITE/convnextv2_base/results2"
    weights_path = "/home/kostas/AMFITRITE/pretrained_model_weights/BIFOLD-BigEarthNetv2-0convnextv2_base-s2-v0.2.0/model.safetensors"
    Logger_path = output_path
    batch_size = 8
    num_workers = 4
