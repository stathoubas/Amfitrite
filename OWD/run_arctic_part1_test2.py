# -*- coding: utf-8 -*-
"""
Created on Sat Mar  7 22:11:43 2026
...
def run_decoupled_prescreener(csv_path, output_dir, x_days, y_days, purity_days, gap_days, n_cases):
    ...
    if 'id_x' not in df.columns and 'HAB_ID' not in df.columns:
        df['generated_id'] = [f"Row_{i}" for i in range(len(df))]
        id_col = 'generated_id'
    else:
        id_col = 'id_x' if 'id_x' in df.columns else 'HAB_ID'

    qty_col = next((c for c in df.columns if 'individual' in c.lower() or 'organism' in c.lower() or 'individu' in c), None)
    ...
    if total_cleans_found < total_habs_found:
        ...
        for cid in valid_hab_clusters:
            ...
            # FIX 1: .dt.normalize() strips the hours/minutes so math is perfectly whole days
            ...
            # Scenario A: Calculate Midpoints of large gaps
            if len(all_dates) > 1:
            ...
            # FIX 2: Dynamic padding based on user input, ensuring they easily clear the safety check
            ...
            # Attempt to pull satellite images for these pristine dates
            for cand_date in candidate_dates:
                if total_cleans_found >= total_habs_found: break
                
                # Final safety check: ensure the date is strictly >= (gap_days / 2) from ANY known record
                # Using a 1-day grace period just to be hyper-safe against leap-year or time-zone weirdness
        ...
        # Append all new imputed rows to the master DataFrame
...
if __name__ == "__main__":
    ...
    n_cases, x_days, y_days, purity_days, gap_days = 2, 14, 5, 10, 120
    run_decoupled_prescreener(r"C:\Users\KostasPikounis\OneDrive_Inlecom_Personal\OneDrive - INLECOM\Amfitrite\task2\OWD\North American Arctic region\homogenized_arctic_plankton_v1.csv", r"C:\Users\KostasPikounis\OneDrive_Inlecom_Personal\OneDrive - INLECOM\Amfitrite\task2\OWD\North American Arctic region", x_days, y_days, purity_days, gap_days, n_cases)
