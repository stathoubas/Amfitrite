...
if __name__ == "__main__":
    ...
    train_loader = torch.utils.data.DataLoader(train_ds, batch_size=8, shuffle=True, num_workers=0)
    val_loader = torch.utils.data.DataLoader(val_ds, batch_size=8, shuffle=False, num_workers=0)
    test_loader = torch.utils.data.DataLoader(test_ds, batch_size=8, shuffle=False, num_workers=0)

    # --- 2. SETUP MODEL & LOGGER ---
    model = HABLightningModel(mode="generic", weights_path = None, lr=1e-5)
    #model = HABLightningModel(mode='s2', weights_path='resnet18_s2.pth')
    
    logger = CSVLogger("logs", name="hab_experiment")
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        save_top_k=1,
        save_last=True,
        filename="best-hab-{epoch:02d}-{val_f1:.2f}"
    )
    ...
    # We use Batch=8, Channels=12, Size=256x256
    summary(model, input_size=(8, 12, 256, 256))

    print("\n--- Generating Architecture Diagram ---")
    output_path = r"C:\Users\KostasPikounis\OneDrive_Inlecom_Personal\OneDrive - INLECOM\Amfitrite\task2\IWD CNN\res18\results2"
    ...
    plot_training_history(metrics_path, output_dir=r"C:\Users\KostasPikounis\OneDrive_Inlecom_Personal\OneDrive - INLECOM\Amfitrite\task2\IWD CNN\res18\results2")
    ...
    train_eval_loader = torch.utils.data.DataLoader(train_ds, batch_size=8, shuffle=False)
    
    evaluate_split(best_model, train_eval_loader, DEVICE, split_name="Train", output_dir=r"C:\Users\KostasPikounis\OneDrive_Inlecom_Personal\OneDrive - INLECOM\Amfitrite\task2\IWD CNN\res18\results2")
    evaluate_split(best_model, val_loader, DEVICE, split_name="Validation", output_dir=r"C:\Users\KostasPikounis\OneDrive_Inlecom_Personal\OneDrive - INLECOM\Amfitrite\task2\IWD CNN\res18\results2")
    evaluate_split(best_model, test_loader, DEVICE, split_name="Test", output_dir=r"C:\Users\KostasPikounis\OneDrive_Inlecom_Personal\OneDrive - INLECOM\Amfitrite\task2\IWD CNN\res18\results2")
    
    print("\n--- Saving Optimal Model Weights ---")
    # Get the path for the raw weights
    weights_path = os.path.join(r"C:\Users\KostasPikounis\OneDrive_Inlecom_Personal\OneDrive - INLECOM\Amfitrite\task2\IWD CNN\res18\results2", "optimal_weights.pth")
