...
# =============================================================================
# 2. EXISTING FUNCTIONS (PRESERVED & PATCHED)
# =============================================================================
...
def update_excel_report(folder_path, excel_path="Master_Report.xlsx", uids_tracker_path="uids_processes.xlsx"):
    ...
    if not uid_lock.acquire():
        print(f"ERROR: could not acquire UID lock ({uids_file})")
        return
    ...
    try:
        ...
        if new_uids:
            ...
            print(f"UID tracker updated ({len(new_uids)} new)")
    finally:
        uid_lock.release()
    ...
    if not excel_lock.acquire():
        print(f"ERROR: could not acquire Excel lock ({excel_file})")
        return
    ...
    try:
        ...
        # Ensure required columns (legacy-safe)
        for col in new_row:
        ...
        summary_df = df.drop(columns=["pred_visual"], errors="ignore")
       ...
       # 3. Save Master Report (With Images)
        with pd.ExcelWriter(excel_file, engine="xlsxwriter") as writer:
            ...
            if "pred_visual" in df.columns:
                col_idx = df.columns.get_loc("pred_visual")

                for i, row in df.iterrows():
                    row_path = Path(row["source_path"])
                    img_path = row_path / "cyfi_prediction_map.png"

                    if img_path.exists():
                        img_data, width, height = compress_image_to_bytes(
                            img_path, (200, 200)
                        )
                        if img_data:
                            worksheet.set_row_pixels(i + 1, 150)
