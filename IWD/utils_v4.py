...
1) per_clouds in Master_Report.xlsx is not filled. Bug fixed.
2) added a column named water_pixels, with the number of pixels classified as water in SCL.
...
def update_excel_report(folder_path, excel_path="Master_Report.xlsx", uids_tracker_path="uids_processes.xlsx"):
    ...
    summary_file = excel_file.parent / "Master_summary.xlsx" # NEW: Summary file
    ...
    if points_data:
        for p in points_data:
            if "uid" in p: current_uids.add(str(p["uid"]))
            if "abun" in p: abun_list_raw.append(p["abun"])
    ...
    new_row = {
        ...
        "per_clouds": meta.get("per_clouds", "N/A"), # FIXED: Now reads from updated metadata
        "water_pixels": meta.get("water_pixels", "N/A"), # NEW
        "high_pred": meta.get("High counts", -1),
        "mod_pred": meta.get("Moderate counts ", -1),
        "low_pred": meta.get("Low counts ", -1),
        # Removed NDVI, NCVI
        "pred_visual": "", # RENAMED from pred_vislual
        "source_path": str(folder)
    }
    
    # 1. Update/Create DataFrame
    if excel_file.exists():
        df = pd.read_excel(excel_file)
        if 'source_path' not in df.columns: df['source_path'] = ""
        # Check if columns exist, if not add them (handling legacy files)
        for col in ["water_pixels", "abun_list", "pred_visual"]:
    ...
    # 2. Save Summary (Data Only)
    df.to_excel(summary_file, index=False)
    print(f"Summary Saved: {summary_file}")

    # 3. Save Master Report (With Images)
    try:
        with pd.ExcelWriter(excel_file, engine='xlsxwriter') as writer:
            ...
            # Updated Image Map (Only pred_visual)
            image_map = {
                "pred_visual": "cyfi_prediction_map.png"
            }
            ...
            for index, row in df.iterrows():
                ...
                # Set row height only if we are inserting an image
