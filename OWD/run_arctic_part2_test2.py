# -*- coding: utf-8 -*-
"""
Created on Thu Mar 12 16:57:38 2026
...
def run_heavy_pipeline(csv_path, output_root, model_paths):
    ...
    for index, row in df.iterrows():
        ...
        # --- ID Fallback Logic for the Merged Dataset ---
        raw_id = row.get('num')
        if pd.isna(raw_id): 
            raw_id = row.get('id_x')
        if pd.isna(raw_id): 
            raw_id = row.get('generated_id', f"Row_{index}")
        ...
        # --- Lat/Lon Fallback Logic for the Merged Dataset ---
        lat_val = row.get('lat')
        if pd.isna(lat_val): lat_val = row.get('decimalLatitude')
        
        lon_val = row.get('lon')
        if pd.isna(lon_val): lon_val = row.get('decimalLongitude')
        ...
        # --- Date Fallback Logic for the Merged Dataset ---
        raw_date = row.get('date')
        if pd.isna(raw_date): raw_date = row.get('Event_Date')
        if pd.isna(raw_date): raw_date = row.get('eventDate')
