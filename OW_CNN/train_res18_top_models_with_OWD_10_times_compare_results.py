...
Target: Open Water (OW)

Runs 10 Monte Carlo Iterations evaluating 3 ResNet-18 variants:
1. Baseline: BigEarthNet -> Trained on OW
2. Zero-Shot: BigEarthNet -> Trained on Master (Universal) -> Evaluated on OW
3. Fine-Tuned: Universal Foundation -> Fine-Tuned on OW
"""
import os
import pandas as pd
import numpy as np
...
def generate_comparison_plots(csv_path, base_output_dir):
    ...
    metrics = [
        ...
        ('ow_hab_acc', 'OW HABs Accuracy'),
        ('ow_nonhab_acc', 'OW non-HABs Accuracy'),
...
if __name__ == "__main__":
    ...
    BASE_OUTPUT_DIR = "/home/kostas/AMFITRITE/IW_and_OW_CNN/OW_after_OW_and_IW_comparisons"
    ...
    # ==========================================================================
    # THE LEAK-FREE 10-ITERATION NESTED MONTE CARLO LOOP (OW TARGET)
    # ==========================================================================
    for iteration in range(1, TOTAL_ITERATIONS + 1):
        ...
        # 2. Filter the Split to get the OW subset
        # Since the OW dataset inherently includes clouds and land, filtering for 'OW' captures everything needed!
        ...
        # OW Specialized loaders (uses filtered dataset)
        ow_train_ds = UniversalWaterDataset(ow_df, mode='training', num_bands=10)
        ow_val_ds   = UniversalWaterDataset(ow_df, mode='validation', num_bands=10)
        ow_test_ds  = UniversalWaterDataset(ow_df, mode='test', num_bands=10) 
        
        ow_train_loader = torch.utils.data.DataLoader(ow_train_ds, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS)
        ow_val_loader   = torch.utils.data.DataLoader(ow_val_ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS)
        ow_test_loader  = torch.utils.data.DataLoader(ow_test_ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS)

        iter_results = {'iteration': iteration}

        # ----------------------------------------------------------------------
        # PHASE 1: Train Baseline (BigEarthNet -> Trained on OW)
        # ----------------------------------------------------------------------
        print("\n[PHASE 1] Training Baseline BigEarthNet ResNet-18 on OW...")
        model_base = HABLightningSystem(
            architecture='resnet18', num_bands=10, mode='bigearthnet',
            weights_path=BIGEARTH_WEIGHTS, lr=1e-4, class_weights=ow_weights
        )
        ...
        trainer_base.fit(model_base, ow_train_loader, ow_val_loader)
        
        best_base = HABLightningSystem.load_from_checkpoint(ckpt_cb_base.best_model_path, class_weights=ow_weights).to(DEVICE)
        metrics_base = evaluate_split(best_base, ow_test_loader, DEVICE, "Test_Baseline", iter_dir)
        ...
        best_found = HABLightningSystem.load_from_checkpoint(best_found_path, class_weights=ow_weights).to(DEVICE)
        
        print("\n[Evaluating Zero-Shot Foundation on OW Test Set]...")
        metrics_zero = evaluate_split(best_found, ow_test_loader, DEVICE, "Test_ZeroShot", iter_dir)
        ...
        # ----------------------------------------------------------------------
        # PHASE 3: Train Specialized Model (Foundation -> Fine-Tuned on OW)
        # ----------------------------------------------------------------------
        print("\n[PHASE 3] Fine-Tuning Specialized Model from Foundation Checkpoint on OW...")
        model_ft = HABLightningSystem(
            architecture='resnet18', num_bands=10, mode='custom_r18',
            weights_path=best_found_path, lr=1e-5, class_weights=ow_weights
        )
        ...
        trainer_ft.fit(model_ft, ow_train_loader, ow_val_loader)
        
        best_ft = HABLightningSystem.load_from_checkpoint(ckpt_cb_ft.best_model_path, class_weights=ow_weights).to(DEVICE)
        metrics_ft = evaluate_split(best_ft, ow_test_loader, DEVICE, "Test_FT_Foundation", iter_dir)
        ...
        del found_train_loader, found_val_loader, ow_train_loader, ow_val_loader, ow_test_loader
        gc.collect()
