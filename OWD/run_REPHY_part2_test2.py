# -*- coding: utf-8 -*-
"""
Created on Fri Mar 27 15:38:28 2026
...
# Import functions from your updated utils script
from utils_v2 import (
    extract_master_scene,
...
import torch
# Force PyTorch's CPU operations to only use 6 threads
...
def run_heavy_pipeline(input_path, output_root, model_paths):
    ...
    # 2. Read the Input Data (Robust Loader for Excel & French CSVs)
    try:
    ...
    for index, row in df.iterrows():
        ...
        # --- ID Fallback Logic for REPHY Dataset ---
        raw_id = row.get('SampleID')
        if pd.isna(raw_id): raw_id = row.get('Résultat : Identifiant')
        if pd.isna(raw_id): raw_id = row.get('Lieu de surveillance : Identifiant')
        if pd.isna(raw_id): raw_id = row.get('num')
        if pd.isna(raw_id): raw_id = row.get('id_x')
        if pd.isna(raw_id): raw_id = row.get('generated_id', f"Row_{index}")
        ...
        if not sat_item_id or sat_item_id.lower() == 'nan':
            # Only log it if it's the first few to avoid flooding the console
            if index < 5:
        ...
        # --- Lat/Lon Fallback Logic (Handling French commas) ---
        lat_val = row.get('decimalLatitude')
        if pd.isna(lat_val): lat_val = row.get('lat')
        
        lon_val = row.get('decimalLongitude')
        if pd.isna(lon_val): lon_val = row.get('lon')
        
        try:
            # Replace comma with dot in case Excel localized the floats as strings
            lat = float(str(lat_val).replace(',', '.'))
            lon = float(str(lon_val).replace(',', '.'))
        except (ValueError, TypeError):
            logging.error(f"Row {index} ({case_id}): Invalid or missing Lat/Lon '{lat_val}', '{lon_val}'. Skipping.")
            continue
            
        # --- Date Fallback Logic (Handling French DD/MM/YYYY) ---
        raw_date = row.get('eventDate')
        if pd.isna(raw_date): raw_date = row.get('Event_Date')
        if pd.isna(raw_date): raw_date = row.get('date')
        
        try:
            # dayfirst=True handles dates like 26/04/2021
        ...
        try:
            ...
            # --- STAGE 2: Extract Master Scene ---
            failed_stage = "2_Extract_Master_Scene"
            extract_ok = extract_master_scene(
                item=selected_item, 
                lat=lat, 
                lon=lon, 
                case_id=uid,
            ...
            if not extract_ok:
                raise ValueError("Master scene extraction failed. Bounds likely outside image.")

            # --- STAGE 3: CyFi Pipeline & Tile Slicing ---
