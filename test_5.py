...
differences compared to test_5
...
import rioxarray
from utils_v5 import update_uids_and_summary, predict_using_cyfi_pipeline, extract_and_save_tile, select_item, get_bounding_box
...
for iii, sel_case in enumerate(low_sev_cases):
    ...
    for item_id, group in processing_groups.items():
        ...
        if not extract_success:
            print(f"----------> Extraction failed for item {item_id}. Skipping CyFi.")
        ...
        if folder_ok:
            ...
            update_uids_and_summary(folder_path=new_path, uids_file=processed_uids_path_name, summary_file=summary_file_path_name)
