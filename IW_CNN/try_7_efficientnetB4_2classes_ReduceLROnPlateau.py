...
class HABDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tensor = torch.nn.functional.interpolate(
            tensor, size=(380, 380), mode='bilinear', align_corners=False
        ).squeeze(0)
...
class HABLightningModel(L.LightningModule):
    ...
    def _build_model(self, mode, weights_path):
        print(f"--- Building EfficientNet B4 (Mode: {mode}) ---")
        
        # 1. Create EfficientNet
        # 'efficientnet_b0' is the model name.
        # pretrained=True downloads ImageNet weights automatically.
...
if __name__ == "__main__":
    ...
    output_path = "/home/kostas/AMFITRITE/efficientnet_B4/results1"
    weights_path = "JUNK"
    Logger_path = output_path
    batch_size = 16
    num_workers = 4
    ...
    model = HABLightningModel(mode='generic', lr=1e-4, weights_path=weights_path)
    ...
    summary(model, input_size=(batch_size, 12, 380, 380))
    ...
    try:
        ...
        model_graph = draw_graph(
            model, 
            input_size=(batch_size, 12, 224, 224), 
            expand_nested=True,
