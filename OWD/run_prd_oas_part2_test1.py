# -*- coding: utf-8 -*-
"""
Created on Wed Mar  4 23:29:11 2026
...
# 3. Process Row by Row
    for index, row in df.iterrows():
        # Checks for HAB_ID first, then id_x, then falls back to Row number
        raw_id = row.get('HAB_ID', row.get('id_x', f"Row_{index}"))
        ...
        # Handles Excel truncating the column names
        lat_col = 'decimalLatitude' if 'decimalLatitude' in row else 'decimalLa'
        lon_col = 'decimalLongitude' if 'decimalLongitude' in row else 'decimalLo'
        ...
        try:
            ...
            if not extract_ok:
                raise ValueError("Tile extraction failed.")
