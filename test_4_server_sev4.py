...
low_sev = after2017_near_water[(after2017_near_water.severity == 4)].copy()
...
for iii, sel_case in enumerate(low_sev_cases):
    ...
    for item_id, group in processing_groups.items():
        ...
        if folder_ok:
            summary_file = os.path.join(sat_test_folder, "Master_Report.xlsx")
            processed_tracker = uids_processed_path
            
            # This handles locking internally now
            update_excel_report(new_path, excel_path=summary_file, uids_tracker_path=processed_tracker)
