# -*- coding: utf-8 -*-
"""
Created on Sun Mar  8 23:59:16 2026
...
def run_heavy_pipeline(csv_path, output_root, model_paths):
    ...
    # Dynamically locate Lat/Lon columns (Handles Excel truncation like 'decimalLa')
    dec_cols = [c for c in df.columns if 'decima' in c.lower()]
    if len(dec_cols) >= 2:
        lat_col, lon_col = dec_cols[0], dec_cols[1]
    else:
        lat_col = next((c for c in df.columns if 'lat' in c.lower()), 'decimalLatitude')
        lon_col = next((c for c in df.columns if 'lon' in c.lower()), 'decimalLongitude')
    
    # 3. Process Row by Row
    for index, row in df.iterrows():
        # --- ROBUST ID EXTRACTION ---
        # Look for id_x first, then HAB_ID, then generated_id from Phase 1. Fallback to Row index.
        raw_id = row.get('id_x', row.get('HAB_ID', row.get('generated_id', f"Row_{index}")))
        ...
        if not sat_item_id or sat_item_id.lower() == 'nan':
            # Suppress logs for empty rows unless it's a multiple of 1000
            if index % 1000 == 0:
        ...
        # Extract Date (Safely handles OBIS M/D/YYYY and imputed YYYY-MM-DD formats)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Phase 2: OBIS Heavy Execution Pipeline")
